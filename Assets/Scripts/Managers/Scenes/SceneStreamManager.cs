...
public class SceneStreamManager : MonoBehaviour
{
    ...
    private static SceneStreamManager s_instance = null;
    ...
    private static SceneStreamManager instance
    {
        get
        {
            lock (s_lock)
            {
                if (s_instance == null)
                {
                    s_instance = FindObjectOfType(typeof(SceneStreamManager)) as SceneStreamManager;
                    if (s_instance == null)
                    {
                        s_instance = new GameObject("Scene Loader", typeof(SceneStreamManager)).GetComponent<SceneStreamManager>();
                    }
                }
                return s_instance;
            }
    ...
    void OnSceneLoadedWrapper(Scene scene, LoadSceneMode mode)
    {
        ...
        StartCoroutine(OnSceneLoaded(scene.name));
    }

    IEnumerator OnSceneLoaded(string sceneName)
    {
